...
# -*- coding: utf-8 -*-
"""
vision_pinsight.py — PiNSIGHT (DepthAI) adapter with NO-CROP preview + correct overlays (1080p wide FOV).

- Forces THE_1080_P sensor mode (widest FOV on cams without 4K).
- Uses ISP downscale to 1280x720 (no crop), so preview shows the full scene.
- NN input is letterboxed to 300x300 in BGR planar (correct byte size), so no tensor size errors.
- Overlays are mapped back to the full video, accounting for letterboxing.
- Publishes (offset_x, area, quality) to VisionBus for Layer-5.
- Auto-downloads the MobileNet-SSD blob via blobconverter if missing.
"""
...
# Reuse your VisionBus
from rc_layer5 import VisionBus
...
class PiNSightDepthAI(threading.Thread):
    ...
    def _build_pipeline(self) -> "depthai.Pipeline":
        ...
        # 1920x1080 -> 1280x720 is exactly 2/3 scale
        cam.setIspScale(2, 3)
        if self.verbose:
    ...
    def _unletterbox(self, x: float, y: float) -> Tuple[float, float]:
        """
        Map NN-normalized coords (0..1, INCLUDING padding) back to video-normalized coords (0..1).
        Handles both horizontal/vertical letterboxing.
        """
                        ...
                        if fb is not None:
                            ...
                            if last_best:
                                ...
                                if _HAVE_CV2:
                                    ...
                                    cx_v = (x1 + x2) * 0.5 / W
                                    offx = (cx_v - 0.5) * 2.0
                                    cv2.putText(frame, f"conf={conf:.2f} area={area:.3f} offx={offx:+.2f}",
